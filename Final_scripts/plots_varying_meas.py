...
for j in tqdm(range(len(meas))):
    ...
    with prog.context as q:
        ...
        ops.Sgate(s) | q[1]
        ops.BSgate() | (q[0], q[1])
        ops.MeasureHomodyne(np.pi/2, select = meas[j]) | q[0]
    ...
    if n > 1:
        for i in range(n-1):
            ...
            with prog2.context as q:
                ...
                ops.BSgate() | (q[0],q[1])
                ops.MeasureHomodyne(np.pi/2, select = meas[j]) | q[1]
...
import matplotlib.pyplot as plt
meas = np.linspace(-4,4,200)
fid_progress = np.load('fide.npy', allow_pickle=True)

with plt.style.context(['science']):
    plt.figure()
    plt.plot(meas, fid_progress, '.', markersize = 3.0, color = 'cadetblue')
    plt.plot(meas, fid_progress, '-', linewidth = 0.5, color = 'lightblue')
    plt.ylabel('Fidelity')
    plt.xlabel(r'Measurement $p$')
